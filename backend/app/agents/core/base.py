"""
CIVICFLOW AI — Reusable Base Agent Framework
Governs execution lifecycle: input validation, execution timing, timeout, retries,
confidence metrics, fallback heuristics, and automated immutable audit logging.
"""
...
logger = logging.getLogger("civicflow.agents.core")
...
class BaseAgent(ABC, Generic[InputT, OutputT]):
    ...
    async def _execute_core(self, input_data: InputT) -> OutputT:
        """Core AI or heuristic reasoning implemented by specialized agent subclass."""
        pass

    @abstractmethod
    def _fallback_heuristic(self, input_data: InputT, error: Optional[Exception]) -> OutputT:
        """Deterministic domain-grounded fallback when external LLM is offline or times out."""
        pass
