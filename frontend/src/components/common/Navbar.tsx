...
import { 
  ...
  Sparkles, Compass, AlertTriangle, Layers 
} from 'lucide-react';

interface NavbarProps {
  currentTab: 'citizen' | 'municipal' | 'admin';
  setCurrentTab: (tab: 'citizen' | 'municipal' | 'admin') => void;
        ...
        <div className="flex items-center gap-3">
          <form onSubmit={handleSearchSubmit} className="relative hidden md:block">
            ...
            <input
              ...
              className="pl-9 pr-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs w-44 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
            />
          </form>

          <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-emerald-50 border border-emerald-200/60 text-emerald-700 text-xs font-medium">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
            </span>
            <span className="hidden lg:inline">6 Agents Active</span>
          </div>
        </div>
      </div>
    </header>
